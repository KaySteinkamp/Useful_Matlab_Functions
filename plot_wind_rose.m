function varargout = plot_wind_rose(D, F, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plot a wind rose of direction and intensity
%
%   Usage:
%      handles = plot_wind_rose(D, F, varargin)
%
%   Inputs:
%      D   Directions
%      F   Intensities
...
%               -this is adjusted so that default is meteo (AMSM 3/8/10)
...
% directions (=wind coming from) conversion:
...
% calculate instensity subdivisions:
if isempty(Ag)
    % gen Ag:
...
for i=1:ncircles
    ...
    circles(i)=plot(x,y,':','color',lineColors); %#ok<*AGROW>
...
for i=1:length(Ay)-1
    if n(i)
        ...
        for j=1:length(Ag)-1
            ...
            if E(i,j)>0, handles(end+1)=fill(x,y,cor{jcor},'edgecolor','k'); end
            r1=r2;
        end
    end
end
